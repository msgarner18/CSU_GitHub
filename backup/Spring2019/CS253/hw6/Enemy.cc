...
{
	reallignParagraph(s, maxSpacing, out);
...
bool operator==(const Enemy &en1, const Enemy &en2)
{
	...
	for(size_t i = 0; i < en1.s.size(); i++)
	{
		enemy1.push_back(en1.s[i]);
	}
	for(size_t i = 0; i < en2.s.size(); i++)
	{
		enemy2.push_back(en2.s[i]);
	}
	for(size_t i = 0; i < enemy1.size() && i < enemy2.size(); i++)
	{
		if(enemy1[i] != enemy2[i])
		{
			areEqual = false;
		}
	}
...
pair<string, string> Enemy::operator[](const size_t index)
{
	pair<string,string> returnValue;
	string line = this->s[index];
	size_t firstSpace = line.find(" ");
	string firstWord = line.substr(0, firstSpace);
	string secondWord = line.substr(firstSpace+1, line.length()-firstSpace);
	returnValue.first = firstWord;
	returnValue.second = secondWord;

	return returnValue;
}

const pair<string, string> Enemy::operator[](const size_t index)const
{
	pair<string,string> returnValue;
	string line = this->s[index];
	size_t firstSpace = line.find(" ");
...
string Enemy::operator[](const string key)
{
	string line = "";
	size_t firstSpace = 0;
	string firstWord = "";
	...
	for(index = 0; index < s.size() && firstWord.compare(key) != 0; index++)
	{
		line = this->s[index];	
		firstSpace = line.find(" ");
		firstWord = line.substr(0, firstSpace);
		secondWord = line.substr(firstSpace+1, line.length()-firstSpace);
	}
...
const string Enemy::operator[](const string key)const
{
	string line = "";
	size_t firstSpace = 0;
	string firstWord = "";
